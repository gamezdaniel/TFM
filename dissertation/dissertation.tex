...
The Small Medium Enterprise (SME) sector que requiere de acceso a sistemas informaticos para sus operaciones viene siendo atraido cada vez mas por los esquemas de cloud computing. Esta demanda viene siendo atendida por grandes proveedores de este tipo de servicios, pero tambien por otras SME que asumen modelos de negocio estrategicos ante este nicho de negocio. Al proveer servicios de nube privada un elemento primordial (major) es la fiabilidad (reliability) de los mismos, trust in that the service consistently performs according to its specifications, y un elemento clave para ello es la disponibilidad de los sistemas. Existen multiples formas de proveer HA a los servicios que se ofrecen, y la eleccion de las herramientas adecuadas no es una tarea facil. 

Netnovation es un buen ejemplo de este tipo de SME, siendo una empresa que se ha basado en consulting around F$\ell$OSS, providing Infrastructure as a Service (IaaS) and cloud services. Al igual que muchos otros, se ve en la necesidad de proveer HA a los sistemas que ofrece con el fin de garantizar los SLA establecidos con sus clientes. Muchas son las opciones, unas mas costosas que otras, con implicaciones economicas, practicas y eticas, pero la mayoria de ellas con un mismo objetivo.

% methods of analysis (Formal language appropriate to report writing)

Resulta necesario aplicar algun metodo que permita la eleccion de la solucion adecuada, considerando los requerimientos establecidos, asi como los recursos de caracter humano y material disponibles para tal fin. Uno de los metodos que ha sido posible encontrar es el ofrecido por Lazy User Model, que focuses on user needs and the demanded effort to chose a solution to a problem. Aunado a esto, se siguen suggested guidelines que afianzan the initial selection and adoption of F$\ell$OSS within the IT infrastructure. Adicionalmente se emplean tecnicas a traves de Metrics Grimoire para el calculo de metricas concretas con las cuales es posible interpretar el comportamiento de los proyectos de F$\ell$OSS que son evaluados como solucion final.

% Findings (Key findings summarised)

Two major alternatives were found, the enterprise embedded solutions offering support plans over the provided integrated product, and also modular or independent F$\ell$OSS which need to be orchestrated in order to provide a final solution. En base a los lineamientos establecidos por Netnovation la solucion final consiste en la integracion de herramientas como Pacemaker, Corosync y DRBD, avoiding disruptive changes on the current operational infrastructure.

% Conclusions (Problem solution summarised) - can be summarized/bulleted
...
- This exercise puts in evidence the potential of F$\ell$OSS and its possibilities in the world of private enterprise

% Recommendations - can be summarized/bulleted
% Limitations of the report

Ultimately se discute hacerca de los cloud-oriented systems and their implications. Is important to note that a comprehensive analysis can be conducted through specialized tools able to provide metrics more specific than those obtained in this research, making reference to them. Also is considered for future work the adoption of technologies that can handle other high availability schemes for cloud services.
...
\indent query = ``select year(date), month(date), day(date), count(*) from scmlog group by year(date), \textbackslash\\
\indent month(date), day(date)"\\
